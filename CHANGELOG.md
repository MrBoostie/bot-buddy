...
- Added preflight strict-tool coverage for commands that exist but are not executable, preventing silent permission-related false positives.
- Added preflight strict-tool coverage for command-probe timeouts, preventing hung tooling checks from being treated as healthy.
...
- Operator command hardening + observability improvements:
  ...
  - Operator audit stream now records `/reload` outcomes (applied, dry-run ok/issues, rejected, rate-limited, invalid usage, applied-with-issues).
...
- Added regression coverage that pins CI usage of both `npm run verify:quick` and `npm run verify`.
