...
Unknown slash commands now return an explicit hint (`unknown command: /<name> (use /help)`) instead of falling through to LLM mode, with near-match suggestions for simple typos (e.g. `/hepl` -> `did you mean /help?`).
